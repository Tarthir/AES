...
class AES:
    ...
    inv_rgf_matrix = [[0x0e, 0x0b, 0x0d, 0x09],
                      [0x09, 0x0e, 0x0b, 0x0d],
                      [0x0d, 0x09, 0x0e, 0x0b],
                      [0x0b, 0x0d, 0x09, 0x0e]]
    rgf_matrix = np.array(rgf_matrix)
    inv_rgf_matrix = np.array(inv_rgf_matrix)
    ...
    # TODO this func and subbytes are pretty much the same
    ...
    def subBytes(self, my_state, s_box=s_box):
        for i in range(4):
            for j in range(4):
                r = 0
    ...
    def mix_columns(self, my_state, matrix=rgf_matrix):
        ...
        for col in range(4):
            for row in range(4):
                new_state[row, col] = self.__mult(matrix[row, :], my_state[:, col])  # the new column replaces c
        return new_state
    ...
    # helper function, gets the correct index into the expanded key for add_round_key
